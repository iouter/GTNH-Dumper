...
public class StructureHacks {
    ...
    static {
        ...
        addTieredElement(
            CHANNEL_ELEMENT = channelElem.getClass()
                .getName());
        IStructureElement<?> onElementPassElem = StructureUtility.onElementPass(o -> {}, elem);
        ON_ELEMENT_PASS = onElementPassElem.getClass()
            .getName();

        try {
            MethodHandles.Lookup lookup = MethodHandles.lookup();
            LAZY_ELEMENT_GETTER = lookup.unreflect(
                ReflectionHelper.findMethod(Class.forName(LAZY_ELEMENT), null, new String[] { "get" }, Object.class));
    ...
    private static <T> ObjectSet<ItemStack> extractTieredBlocks(T multi, IStructureElement<T> element,
        ...
        ObjectSet<ItemStack> result = new ObjectLinkedOpenHashSet<>();
